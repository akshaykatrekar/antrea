...
For any given release `<TAG>` (e.g. `v0.3.0`), you can get the Antrea IPsec
deployment yaml at:
```
...
`antrea-ipsec` to save the PSK string. For security consideration, we recommend
...
would use. Check the `antrea-ipsec` Secret spec below:
```
---
apiVersion: v1
kind: Secret
